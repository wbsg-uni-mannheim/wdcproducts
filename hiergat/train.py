    ...
    while ((epoch <= args.n_epochs) and (no_improvement_count <= 10)):
        ...
        if dev_f1 > 1e-6:
            ...
            if (epoch == args.n_epochs):
                path = os.getcwd()
                path = path + '/output/' + str(run_tag) + '.txt'
                os.makedirs(os.path.dirname(path), exist_ok=True)

                dict = {'best_test_f1': best_test_f1}

                with open(path, "a+") as f:
                    f.write(repr(dict) + '\n')

            print("current_best_test_f1: " + str(best_test_f1))
...
if __name__ == "__main__":
    ...
    run_tag = '%s_lr=%s_id=%d_batch=%d' % (task, args.lr, args.run_id, args.batch_size)
