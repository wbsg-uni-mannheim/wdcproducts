...
def main():
    ...
    if training_args.do_train:
        ...
        if training_args.evaluation_strategy != 'no':
            ...
            validation_dataset = ContrastiveClassificationDataset(validation_dataset, dataset_type='validation', size=data_args.train_size, tokenizer=model_args.tokenizer, dataset=data_args.dataset_name, additional_data=data_args.additional_data, only_additional=data_args.only_additional, only_title=data_args.only_title)
        if training_args.load_best_model_at_end:
            test_dataset = raw_datasets["test"]
            test_dataset = ContrastiveClassificationDataset(test_dataset, dataset_type='test', size=data_args.train_size, tokenizer=model_args.tokenizer, dataset=data_args.dataset_name, additional_data=data_args.additional_data, only_additional=data_args.only_additional, only_title=data_args.only_title)
            if data_args.dataset_name == 'lspc' and 'wdcproducts' not in raw_datasets["train"]:
                swc_test_dataset = ContrastiveClassificationDataset('../../data/interim/wdc-lspc/gold-standards/preprocessed_computers_new_testset_1500.pkl.gz', dataset_type='test', size=data_args.train_size, tokenizer=model_args.tokenizer, dataset=data_args.dataset_name, additional_data=data_args.additional_data, only_additional=data_args.only_additional, only_title=data_args.only_title)
