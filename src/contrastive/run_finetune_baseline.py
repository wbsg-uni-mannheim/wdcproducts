...
class CustomTrainer(Trainer):
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        loss_fct = nn.CrossEntropyLoss(weight=torch.tensor([1.0, self.pos_neg]).to(self.model.device))
...
def main():
    ...
    if training_args.do_train:
        ...
        train_dataset = BaselineClassificationDataset(train_dataset, dataset_type='train', size=data_args.train_size, tokenizer=model_args.tokenizer, dataset=data_args.dataset_name, aug=data_args.augment, additional_data=data_args.additional_data, only_additional=data_args.only_additional, only_title=data_args.only_title)
        if training_args.evaluation_strategy != 'no':
            validation_dataset = raw_datasets["validation"]
            validation_dataset = BaselineClassificationDataset(validation_dataset, dataset_type='validation', size=data_args.train_size, tokenizer=model_args.tokenizer, dataset=data_args.dataset_name, additional_data=data_args.additional_data, only_additional=data_args.only_additional, only_title=data_args.only_title)
        if training_args.load_best_model_at_end:
            test_dataset = raw_datasets["test"]
            test_dataset = BaselineClassificationDataset(test_dataset, dataset_type='test', size=data_args.train_size, tokenizer=model_args.tokenizer, dataset=data_args.dataset_name, additional_data=data_args.additional_data, only_additional=data_args.only_additional, only_title=data_args.only_title)
            if data_args.dataset_name == 'lspc' and 'wdcproducts' not in raw_datasets["train"]:
                swc_test_dataset = BaselineClassificationDataset('../../data/interim/wdc-lspc/gold-standards/preprocessed_computers_new_testset_1500.pkl.gz', dataset_type='test', size=data_args.train_size, tokenizer=model_args.tokenizer, dataset=data_args.dataset_name, additional_data=data_args.additional_data, only_additional=data_args.only_additional, only_title=data_args.only_title)
            if data_args.dataset_name == 'lspc' and 'wdcproducts' in raw_datasets["train"]:
        ...
        test_dataset = raw_datasets["test"]
        test_dataset = BaselineClassificationDataset(test_dataset, dataset_type='test', size=data_args.train_size, tokenizer=model_args.tokenizer, dataset=data_args.dataset_name, additional_data=data_args.additional_data, only_additional=data_args.only_additional, only_title=data_args.only_title)
        if data_args.dataset_name == 'lspc' and 'wdcproducts' not in data_args.train_file:
            swc_test_dataset = BaselineClassificationDataset('../../data/interim/wdc-lspc/gold-standards/preprocessed_computers_new_testset_1500.pkl.gz', dataset_type='test', size=data_args.train_size, tokenizer=model_args.tokenizer, dataset=data_args.dataset_name, additional_data=data_args.additional_data, only_additional=data_args.only_additional, only_title=data_args.only_title)
        if data_args.dataset_name == 'lspc' and 'wdcproducts' in raw_datasets["train"]:
                unseen_set_one = BaselineClassificationDataset(raw_datasets["test"].replace('000un', '050un'), dataset_type='test', size=data_args.train_size, tokenizer=model_args.tokenizer, dataset=data_args.dataset_name, additional_data=data_args.additional_data, only_additional=data_args.only_additional, only_title=data_args.only_title)
                unseen_set_two = BaselineClassificationDataset(raw_datasets["test"].replace('000un', '100un'), dataset_type='test', size=data_args.train_size, tokenizer=model_args.tokenizer, dataset=data_args.dataset_name, additional_data=data_args.additional_data, only_additional=data_args.only_additional, only_title=data_args.only_title)
    ...
    for run in range(3):
        ...
        if training_args.do_predict or training_args.do_train:
            ...
            if 'wdcproducts' in raw_datasets["train"]:
                predict_results = trainer.predict(
                    unseen_set_one,
