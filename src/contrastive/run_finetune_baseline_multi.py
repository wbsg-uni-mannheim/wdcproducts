...
class CustomTrainer(Trainer):
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        loss_fct = nn.CrossEntropyLoss(weight=torch.tensor(self.pos_neg).to(self.model.device))
...
def main():
    ...
    def model_init(trial):
        ...
        if model_args.model_pretrained_checkpoint:
            my_model = AutoModelForSequenceClassification.from_pretrained(model_args.tokenizer, num_labels=num_labels)
    ...
    for run in range(3):
        ...
        pos_neg = get_posneg(train_dataset)
        if model_args.model_pretrained_checkpoint:
            model = AutoModelForSequenceClassification.from_pretrained(model_args.tokenizer, num_labels=num_labels)
            if model_args.grad_checkpoint:
