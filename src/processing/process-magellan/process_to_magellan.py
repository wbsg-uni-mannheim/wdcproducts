...
def preprocess_magellan(file, columns_to_preprocess, experiment_name, dataset_name, validation_set=None):
    ...
    if '.pkl.gz' in file:
        ...
        if 'wdcproducts' in file and 'gs' not in file:
            val_df = pd.read_pickle(validation_set)
    ...
    if isinstance(validation_set, type(None)):
        ...
        if 'csv' in validation_set:
            ...
            validation_ids_df = pd.read_pickle(validation_set)
...
if __name__ == '__main__':
    ...
    for file in glob.glob('../../../data/interim/wdc-lspc/training-sets/*'):
        if 'multi' in file or 'wdcproducts' not in file:
            continue
        valid = file.replace('training', 'validation')
        valid = valid.replace('train', 'valid')
        if 'wdcproducts' not in file:
