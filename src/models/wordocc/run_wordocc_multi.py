...
def run_wordcooc(train_set, valid_set, test_set, feature_combinations, classifiers, experiment_name,dataset_name,
    ...
    report_train_name = train_file.replace('.pkl.gz', '').replace('preprocessed_', '')
    report_test_name = test_file.replace('.pkl.gz', '').replace('preprocessed_', '')

    os.makedirs(os.path.dirname(f'../../../reports/{dataset_name}/wordocc/{experiment_name}/'),
                exist_ok=True)

    os.makedirs(os.path.dirname(f'../../../models/{dataset_name}/wordocc/{experiment_name}/'),
                exist_ok=True)


    with open(f'../../../reports/{dataset_name}/wordocc/{experiment_name}/{report_train_name}_{report_test_name}.csv',
              "w") as f:
        f.write(
    ...
    for run in range(1, 4):
        for feature_combination in feature_combinations:
            ...
            val_df = train_original_df[train_original_df['id'].isin(validation_ids_df['id'].values)]
            train_only_df = train_original_df[~train_original_df['id'].isin(validation_ids_df['id'].values)]
            ...
            for k, v in classifiers.items():
                ...
                model = RandomizedSearchCV(cv=ps, estimator=classifier, param_distributions=v['params'],
                                           random_state=run, n_jobs=4, scoring='f1_micro', n_iter=500, pre_dispatch=8,
                                           return_train_score=True)

                feats_train = scipy.sparse.vstack(train_df[feature_combination + '_wordocc'])
                labels_train = train_df['label']
                feats_gs = scipy.sparse.vstack(gs_df[feature_combination + '_wordocc'])
                ...
                model = learner
                feats_train = scipy.sparse.vstack(train_only_df[feature_combination + '_wordocc'])
                ...
                if write_test_set_for_inspection:
                    ...
                    out_path = f'../../../data/processed/inspection/{dataset_name}/wordocc/{experiment_name}/'
                    os.makedirs(os.path.dirname(out_path), exist_ok=True)

                    file_name = '_'.join(
                        [os.path.basename(train_set).replace('preprocessed_', ''), os.path.basename(test_set).replace('preprocessed_', ''), k, feature_combination])
                ...
                dump(model, f'../../../models/{dataset_name}/wordocc/{experiment_name}/{report_train_name}_{report_test_name}_{k}_{feature_combination}_{run}.joblib')

                with open(f'../../../reports/{dataset_name}/wordocc/{experiment_name}/{report_train_name}_{report_test_name}.csv', "a") as f:
                    f.write(feature_combination + '#####' + k + '#####' + str(
                        scores['mean_train_score']) + '#####' + str(scores['std_train_score'])
                            + '#####' + str(scores['mean_test_score']) + '#####' + str(
                        scores['std_test_score']) + '#####' + str(gs_report['accuracy']) + '#####' + str(
                        gs_report['accuracy']) + '#####' + str(gs_report['accuracy'])
...
if __name__ == '__main__':
    ...
    for file in glob.glob('../../../data/processed/wdc-lspc/wordocc/learning-curve/*'):
        ...
        if 'train_' in file and '_gs' not in file:
            ...
            test = '../../../data/processed/wdc-lspc/wordocc/learning-curve/{}'.format(test)
