...
def preprocess_dataset():
    os.makedirs('./ditto/data/final_output/', exist_ok=True)
    os.makedirs('./hiergat/data/final_output/', exist_ok=True)
    ...
    for path in glob.glob(os.path.join(test_path, (r'preprocessed_wdcproducts[!multi]*.pkl.gz'))):
        test_df = preprocess_part(path)
        np.savetxt(f'./ditto/data/final_output/{path[37:-7]}.txt', test_df.values, fmt = "%s")
        np.savetxt(f'./hiergat/data/final_output/{path[37:-7]}.txt', test_df.values, fmt = "%s")
    print(f'DONE TEST')

    for path in glob.glob(os.path.join(valid_path, (r'preprocessed_wdcproducts[!multi]*.pkl.gz'))):
        valid_df = preprocess_part(path)
        np.savetxt(f'./ditto/data/final_output/{path[38:-7]}.txt', valid_df.values, fmt = "%s")
        np.savetxt(f'./hiergat/data/final_output/{path[38:-7]}.txt', valid_df.values, fmt = "%s")
    print(f'DONE VALID')

    for path in glob.glob(os.path.join(train_path, (r'preprocessed_wdcproducts[!multi]*.pkl.gz'))):
        train_df = preprocess_part(path)
        np.savetxt(f'./ditto/data/final_output/{path[36:-7]}.txt', train_df.values, fmt = "%s")
        np.savetxt(f'./hiergat/data/final_output/{path[36:-7]}.txt', train_df.values, fmt = "%s")
    print(f'DONE TRAIN')
